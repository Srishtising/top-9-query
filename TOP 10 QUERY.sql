...
select model_id
from(
select *, row_number() over(partition by model_name order by model_id) as rn 
from cars )
	as c
	where c.rn>1
)	
...
--- Q2: Display highest and lowest salary in each deptartment--- 
...
select *, max(salary) over(partition by dept order by salary desc) as Max_sal, 
min(salary) over(partition by dept order by salary desc range between unbounded preceding and unbounded following) as Min_sal 
from employee;
...
select *, cumulative_distance-lag(cumulative_distance,1,0) over(partition by cars order by days) as last_day from car_travels;
...
with recursive ct1 as
(
	select id,item_name, total_count, 1 as lavel
	from travel_items
	union all
	select ct1.id, ct1.item_name,ct1.total_count-1,lavel+1 as lavel
	from ct1
	join travel_items as t on ct1.item_name=t.item_name and t.id=ct1.id
	where ct1.total_count>1

)

select id, item_name from ct1 order by id;
...
--each team play with other team just once
with ct1 as
(
select *, row_number() over() rn from teams
)

select c1.team_name Team, c2.team_name as Opponent
from ct1 as c1
join ct1 as c2 on c1.rn<c2.rn
order by c1.team_name;


--each team play with other team just twice
with ct1 as
(
select *, row_number() over() rn from teams
)
...
--who comes under the name 'Asha'
with recursive ct1 as
(
	select * from emp_details where name='Asha'
	union
	select e.* from ct1 join emp_details as e on ct1.id=e.manager_id
)
...
with ct1 as
(
select year_id, month_id, to_char(order_date,'MON') AS mon , avg(sales) as avg_sales
from sales_order 
where year_id in (2003,2004)
group by year_id,month_id, to_char(order_date,'MON')
order by year_id, month_id
)
select c1.mon as Months, round(abs(c1.avg_sales-c2.avg_sales)::decimal,2) As Diff
from ct1 as c1
join ct1 as c2
on c1.mon=c2.mon
where c1.year_id=2003 and c2.year_id=2004
...
and exists (select cust_name 
				  ...
				  and d2.status in ('CREATED','SUBMITTED'))
	union
select distinct cust_name, 'AWAITING PROGRESS' as status
from cust_orders as d
where status='SUBMITTED'
and not exists (select cust_name 
				  from cust_orders as d2 
				  where d.cust_name=d2.cust_name
				  and d2.status in ('CREATED','DELIVERED'))
	UNION
select distinct cust_name, 'AWAITING SUBMISSION' as status
from cust_orders as d
